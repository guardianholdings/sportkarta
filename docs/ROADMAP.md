...
- [ ] 5.2 Leaderboards with minor protection enforced at the query layer and attacked in tests — **needs a decision first:** CLAUDE.md currently states "no leaderboard", which is how Stage 3.2 honoured the minors rule. This item and that rule cannot both stand as written.
- [ ] 5.3 Challenges as config with admin CRUD + staged demo campaign
