...
function whenParts(startsAtLocal: string, locale: string): [string, string] {
  ...
  const d = new Intl.DateTimeFormat(locale === 'en' ? 'en-GB' : 'bg-BG', {
    weekday: 'short',
    day: 'numeric',
    month: 'short',
        ...
        {sessions.length === 0 ? (
          ...
          <ul className="flex flex-col gap-2.5">
            {sessions.map((s) => {
              ...
              const Icon =
                s.sport in SPORT_VISUALS
                  ? SPORT_VISUALS[s.sport as CanonicalSport].Icon
                  : SPORT_VISUALS.multi.Icon;
                ...
                <li key={s.occurrenceId}>
                  ...
                  >
                    ...
                    <span className="shrink-0 text-right">
                      <span className="block font-mono text-caption font-medium text-ink">
                        {date}
                      </span>
                      <span className="block font-mono text-caption tabular-nums text-brand">
                        {time}
                      </span>
                    </span>
                  </Link>
